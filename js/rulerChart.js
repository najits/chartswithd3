function RulerChart(config) {
  ...
  function myChart() {
    ...
    d3.csv(p.data, function(error, csv) {
      ...
      svg.selectAll(".circle")
        ...
        .on("mouseout", function() {
            ...
            // Unhighlight legend.
            chart.selectAll(".legendCircle").attr("r", radius.normal);
            chart.selectAll(".legendLabel").style("font-weight", "normal");
      ...
      svg.selectAll(".legendItems")
        ...
        .on("mouseout", function() {
            ...
            // Unanimate circles.
            chart.selectAll(".circle").attr("r", radius.normal);
      ...
      chart.select(".reset")
        .on("click", function() {
            ...
            dimensions.map(function(p) {
              floorXpx[p] = 0;
              x[p].domain(origExtent[p]).range([floorXpx[p], width]);
            });
    ...
    // Set reset button display
    function setResetDisplay (v) {
      var reset = chart.select(".reset");
      if(v) {
    ...
    function centerLine(d) {
      chart.select(".line.centered").select("path")
        .transition()
          .duration(transition.durationShort)
        .attr("d", path(d));
    }
    ...
    function setCenterLine() {
      chart.select(".line.centered").remove();
      chart.select(".line").attr("class", "line centered");
    }
